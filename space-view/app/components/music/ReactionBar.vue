...
const reactions = ref<Record<string, number> | null>(null)
const userReaction = ref<string | null>(null)
const api = useApi()

const loadReactions = async () => {
  try {
    const res: any = await api.get(`/music/reactions?songId=${props.songId}`)
    reactions.value = res.data || res
  } catch (e) {
...
const toggleReaction = async (type: string) => {
  try {
    await api.post(`/music/reactions/toggle`, { songId: props.songId, type })
...
.reaction-btn {
  ...
  &.active { border-color: var(--primary); background: var(--primary-50); color: var(--primary); font-weight: 600; }
}

.reaction-icon { font-size: 1rem; line-height: 1; }

.reaction-count { font-size: 0.78rem; font-weight: 600; min-width: 12px; text-align: center; }
</style>
