...
defineEmits(['claim'])
const questTitles: Record<number, string> = { 1: 'Viết 1 blog', 2: 'Đọc 3 blogs', 3: 'Reaction 5 bài', 4: 'Viết 1 status', 5: 'Viết 3 blogs (tuần)' }
const getQuestTitle = (id: number) => (props.questNames as Record<number, string>)[id] || questTitles[id] || `Nhiệm vụ #${id}`
const completedCount = computed(() => props.quests.filter((q: any) => q.status === 'COMPLETED' || q.status === 'CLAIMED').length)
</script>
