...
.tag-page { max-width: 700px; margin: 0 auto; }
.tag-page h1 { font-size: 1.5rem; font-weight: 700; margin-bottom: 4px; }
.page-desc { color: var(--text-muted); margin-bottom: var(--space-xl); }
.tag-blogs { display: flex; flex-direction: column; gap: var(--space-sm); }
.tag-blog-item { background: var(--surface); border: 1px solid var(--border-light); border-radius: var(--radius-lg); padding: var(--space-md); transition: all var(--duration-fast) ease; }
.tag-blog-item:hover { border-color: var(--primary); }
