...
<script setup>
import { ref, onMounted, onUnmounted, computed, provide, nextTick, watch } from 'vue'
...
const buildAudioList = () => (props.playlist?.songs || []).map(song => ({
  name: song.title,
  artist: song.artist,
...
const initPlayer = async () => {
  ...
  apRef.value.on('play', () => {
    ...
    props.plugins.forEach(p => p.onPlay?.(currentSong.value))
  })
  apRef.value.on('pause', () => {
    isPlaying.value = false
    props.plugins.forEach(p => p.onPause?.(currentSong.value))
  })
  apRef.value.on('listswitch', (index) => {
    currentIndex.value = index
    emit('songChange', currentSong.value)
    nextTick(() => props.plugins.forEach(p => p.onListSwitch?.(index, currentSong.value)))
  })
  apRef.value.on('error', (e) => props.plugins.forEach(p => p.onError?.(e)))

  props.plugins.forEach(p => p.onInit?.(apRef.value))
}
...
const switchTo = (index) => apRef.value?.list?.switch(index)
...
const songActionPlugins = computed(() => props.plugins.filter(p => p.songActions))
const toolbarPlugins = computed(() => props.plugins.filter(p => p.toolbarActions))
const addSongPlugins = computed(() => props.plugins.filter(p => p.addSongSection))
</script>
...
// Playlist manager plugin styles
