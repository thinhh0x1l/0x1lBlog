  ...
  <div class="sidebar-card music-box" v-if="playlist">
    ...
    <div class="youtube-embed" v-if="isPlaying && currentSong?.source === 'youtube'">
      <iframe
        ...
        allowfullscreen
      ></iframe>
      <a :href="`https://www.youtube.com/watch?v=${currentSong?.sourceId}`" target="_blank" class="yt-link">
...
const voteSong = (songId, vote) => {
  ...
  const song = props.playlist?.songs?.find(s => s.id === songId)
  if (song) song.voteCount += vote * 2
}

const removeSong = (songId) => {
  if (!props.playlist?.songs) return
  const idx = props.playlist.songs.findIndex(s => s.id === songId)
  if (idx >= 0) {
...
const submitSong = () => {
  ...
  const sourceId = newSong.value.source === 'youtube' && newSong.value.sourceUrl
    ? (extractYoutubeId(newSong.value.sourceUrl) || 'dQw4w9WgXcQ')
    : `song_${Date.now()}`
  const song = {
    id: Date.now(),
...
.youtube-embed { margin-bottom: 8px; border-radius: var(--radius); overflow: hidden; position: relative; }
.youtube-embed iframe { display: block; border-radius: var(--radius); }
.yt-link { display: inline-flex; align-items: center; gap: 4px; font-size: 0.65rem; color: var(--text-muted); text-decoration: none; margin-top: 4px; }
